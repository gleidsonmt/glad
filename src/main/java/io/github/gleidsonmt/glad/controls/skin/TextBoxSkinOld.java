...
import io.github.gleidsonmt.glad.controls.text_box.TextBoxOld;
...
public class TextBoxSkinOld extends TextBoxBaseSkin {

    private final IconButton clearButton;

    public TextBoxSkinOld(TextBoxOld _control) {
        super(_control);
...
//        _control.actionProperty().addListener((_, _, newValue) -> setRightNode(newValue  ? createRightAction() : null));
    ...
    protected void configAction(TextBoxOld control) {
    ...
    private @NotNull EventHandler<MouseEvent> createAction(TextBoxOld control) {
        return _ -> {
